import {
    ...
    randomId,
    getClasses,
    isShallowObjectDifferent
} from '../utils';
import {fndryFormConfig, fndryFormState, fndryFormParentForm} from '../utils/providers';
import extend from 'extend';

export default {
    render(h) {
        let foundVnodes = getVModelAndLabel(this.$slots.default, this.fndryFormConfig);
        ...
        if (vModelnodes.length) {
            ...
            vModelnodes.forEach((vnode) => {
                ...
                vnode.data.directives.push({
                    name: 'form-validator',
                    value: {fieldstate: this.fieldstate, config: this.fndryFormConfig}
                });
                vnode.data.attrs['form-validator'] = '';
                vnode.data.attrs['debounce'] = this.debounce;
            });
        } else {
            //console.warn('Element with v-model not found');
        }
        return h(this.tag || this.fndryFormConfig.validateTag, {'class': this.className, attrs}, this.$slots.default);
    },
    props: {
        schema: Object,
        state: Object,
    ...
    inject: {fndryFormConfig, fndryFormState, fndryFormParentForm},
    provide() {
        return {
    ...
    computed: {
        className() {
            return this.getClasses(this.fndryFormConfig.validateClasses);
        },
        inputClassName() {
            return this.getClasses(this.fndryFormConfig.inputClasses);
        }
    },
    mounted() {
        ...
        const vModelEls = this.$el.querySelectorAll('[form-validator]');
    ...
    created() {
        this.formstate = this.state || this.fndryFormState;
        const vm = this;
            ...
            _validate(vnode) {
                ...
                Object.keys(this._validators).forEach((validator) => {
                    ...
                    if (typeof result === 'boolean') {
                        ...
                        vm.fndryFormParentForm.promises.push(result);
                    }
                });

                if (pending.promises.length) {
                    vm.fndryFormConfig.Promise.all(pending.promises).then((results) => {
